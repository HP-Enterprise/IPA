...
package com.cmos.ipa.protocol.knx.dptxlator;
...
public final class TranslatorTypes
{
	...
	private static void addTranslator(final int main, final String className, final String desc)
	{
		try {
			map.put(new Integer(main),
					new MainType(main,
							Class.forName("com.cmos.ipa.protocol.knx.dptxlator." + className), desc));
		...
		throws KNXException
	{
		...
		throw new KNXException("main number "+ mainNumber + " not found for " + dptID + ";");
	}
	...
	public static int getMainNumber(final int mainNumber, final String dptID)
	{
