	...
	synchronized public void run() {
		...
		while (running) {
			try {
				...
				connect();
				log.log_info(Translate.getString("JEASYOPC_CONNECTED"));

				// register groups on server
				registerGroups();
				log.log_info(Translate.getString("JEASYOPC_GRP_REG"));

				// run asynchronous mode 2.0
				for (int i = 0; i < groups.size(); i++) {
					asynch20Read(getGroupByClientHandle(i));
				}
				log.log_info(Translate.getString("JEASYOPC_ASYNCH20_START"));
				...
				while (running) {
					...
					if (!connected) {
						throw new ConnectivityException(Translate.getString("CONNECTIVITY_EXCEPTION") + " " + getHost()
								+ "->" + getServerProgID());
					}
		...
		try { // remove groups
			unregisterGroups();
			log.log_info(Translate.getString("JEASYOPC_GRP_UNREG"));
		...
		connected = false;
		running = false;
		log.log_info(Translate.getString("JEASYOPC_DISCONNECTED"));
	}
